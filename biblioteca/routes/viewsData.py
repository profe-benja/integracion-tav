...
from ..models import Libro, Pedido, UserProfile, Categoria, Producto, DetalleBoleta, Boleta
...
def data(request):
    ...
    productos = ['Guitarra electrica', 'cuerda', 'Bateria', 'cables']

    for _ in range(30):
        nombre_producto = productos[random.randint(0, 3)]
        descripcion_producto = fake.paragraph()
        precio_producto = random.randint(1, 100)
        # imagen = 'img/productos/' + str(random.randint(1, 10)) + '.png'
        imagen = 'img/productos/' + img_productos[random.randint(0, 4)]
